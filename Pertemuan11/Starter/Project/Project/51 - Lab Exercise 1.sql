...
(
	SELECT 
		s.custid, od.qty * od.unitprice AS salesvalue, c.categoryname
...
SELECT 
	custid, p.Beverages, p.Condiments,
    p.Confections, p.[Diary Products] , p.[Grains/Cereals],
    p.[Meat/Poultry], Produce, Seafood
FROM SalesByCategory
PIVOT (
	SUM(salesvalue) FOR categoryname 
	IN ( Beverages, Condiments, Confections, [Diary Products], 
